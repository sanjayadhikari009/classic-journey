...
@section('content')
<div class="w-full px-4 md:pl-20 md:pr-8 py-8">
    <div class="bg-white rounded-lg shadow p-8 mb-8">
        <h1 class="text-2xl font-bold mb-6">Edit user</h1>
        <form method="POST" action="{{ route('admin.admin.members.update', $member->id) }}">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label class="block text-sm font-medium">Username*</label>
                <input name="username" type="text" class="form-input w-full border rounded px-3 py-2" value="{{ old('username', $member->username) }}" required>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium">Mobile number*</label>
                <input name="mobile_number" type="text" class="form-input w-full border rounded px-3 py-2" value="{{ old('mobile_number', $member->mobile_number) }}" required>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium">Password</label>
                <input name="password" type="password" class="form-input w-full border rounded px-3 py-2" placeholder="Enter new Password">
            ...
            <div class="mb-4">
                ...
                <input name="referral_code" type="text" class="form-input w-full border rounded px-3 py-2" value="{{ old('referral_code', $member->referral_code) }}">
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium">Referral by*</label>
                <select name="referral_by" class="form-select w-full border rounded px-3 py-2" required>
                    <option value="">Select Referral</option>
                    @foreach($referrals as $ref)
                        <option value="{{ $ref->id }}" @if(old('referral_by', $member->referral_by) == $ref->id) selected @endif>{{ $ref->referral_code }} - {{ $ref->username }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                ...
                <select name="level" id="level-select" class="form-select w-full border rounded px-3 py-2" required>
                    ...
                    @foreach($levels as $level)
                        <option value="{{ $level->level }}" data-normal_commission="{{ $level->normal_commission }}" data-stop_commission="{{ $level->stop_commission }}" data-journey="{{ $level->number_of_journey }}" data-signup_bonus="{{ $level->signup_bonus }}" @if(old('level', $member->level) == $level->level) selected @endif>
            ...
            <div class="flex gap-2">
                <button type="submit" class="btn btn-primary bg-green-600 text-white px-4 py-2 rounded">Save User</button>
                <a href="{{ route('admin.admin.members.index') }}" class="btn btn-secondary bg-red-100 text-red-700 px-4 py-2 rounded">Cancel Edit</a>
            </div>
        </form>
    </div>
</div>
